...
- **TLS clients are platform-native; server accept is OpenSSL except on Darwin.** Per [ADR-0016](./docs/adr/0016-tls-backend-per-platform.md), outbound `TransportSecurity.pas` clients use **SChannel on Windows**, **SecureTransport on macOS**, and runtime-loaded **OpenSSL** elsewhere. Per [ADR-0024](./docs/adr/0024-openssl-server-tls-accept.md), server accept uses runtime-loaded **OpenSSL** on Windows and Unix-not-Darwin; macOS servers use Network.framework outside this package. OpenSSL is never import-linked or shipped in LWPT release archives. The guards in `pr.yml`, `ci.yml`, and `release.yml` inspect Windows PE imports and fail only if `lwpt.exe` import-links `libssl` / `libcrypto`; runtime-loader DLL-name strings are required. Full per-platform story in [`docs/deployment.md`](./docs/deployment.md).
