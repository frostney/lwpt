...
- **HTTPS goes through the `HTTPClient` package** (raw sockets + per-platform client TLS per [ADR-0016](./docs/adr/0016-tls-backend-per-platform.md): SChannel on Windows, SecureTransport on macOS, system OpenSSL on Unix-not-Darwin). Server accept follows [ADR-0024](./docs/adr/0024-openssl-server-tls-accept.md) on Unix-not-Darwin, [ADR-0033](./docs/adr/0033-schannel-server-tls-accept-on-windows.md) (native SChannel) on Windows, and native Secure Transport at the package seam on macOS compatibility runtimes. The registry uses Network.framework by default on macOS 26 and newer, and the shared portable listener plus Secure Transport on macOS 15 and older; dispatch uses the public runtime Darwin kernel release (24 for Secure Transport, 25 and newer for Network.framework), never a synthesized marketing version or CPU architecture, and the compatibility listener accepts only `localhost` or canonical IPv4. Do not switch to `fphttpclient`. The byte-safe `AppendRawBytes` accumulator that fixes the `Copy(PAnsiChar)` truncation bug is non-negotiable.
...
- **Network operations are explicit.** Outbound toolkit network access is limited to `lwpt install` and its manifest-editing frontends `lwpt add` / `lwpt remove` (both run the install transaction per [ADR-0019](./docs/adr/0019-add-remove-subcommands.md)) plus `lwpt outdated` / `lwpt update` ([ADR-0039](./docs/adr/0039-outdated-update-subcommands.md) — tag listing; `update` may also fetch archives). `lwpt registry sync` explicitly pulls signed origin content into a configured mirror ([ADR-0045](./docs/adr/0045-verified-registry-mirror.md)). `lwpt registry serve` is the only toolkit-owned inbound listener and serves its configured origin or mirror until stopped without request-time upstream reads. Test programs own their own network policy; LWPT's live-network programs require `LWPT_ENABLE_NETWORK=1`.
...
- `lwpt repair [--silent]` — Recover project and shared-cache residue
  - `--silent` — Suppress ordinary output and emit only the final command result
- `lwpt registry <init|sync|verify|rotate-key|serve> [--data-dir <path>] [configuration options] [--silent]` — Initialize, synchronize, verify, rotate keys or serve a self-hosted registry
  - `--data-dir=<value>` — Registry data directory (default: .lwpt/registry)
  ...
  - `--role=<value>` — Registry role: origin (default) or mirror (init only)
  - `--upstream=<value>` — Canonical upstream base URL (required for mirror init)
  - `--key-id=<value>` — Pinned origin root ed25519 key ID (required for mirror init)
  - `--public-key=<value>` — Pinned origin root public key in hex: encoding (required for mirror init)
  - `--from-key=<value>` — Expected current signing key ID (required for rotate-key; guards retries)
  - `--silent` — Suppress ordinary output and emit only the final command result
