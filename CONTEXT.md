...
**Lockfile**:
The machine-written schema-v3 record at `lwpt.lock` of every resolved dependency's declared source, resolved ref and commit, canonical source and constraint identity, fetched URL, extracted-tree hash, and archive hash. Generated by `lwpt install`; consumed by `lwpt install --frozen`. Never hand-edited.
...
The optional `include = [...]` and `exclude = [...]` glob arrays on a dependency. They select the filtered snapshot published from a fetched archive or local source. The retired `subdir` field is a hard error; use an include such as `src/middleware/**` instead.
...
**Toolkit state**:
Everything under `.lwpt/` at a project's root. `modules/` and `archives/` are committed zero-install state; `tmp/` is the gitignored install workspace and rollback journal; `sessions/` is gitignored invocation-private compiler staging. `install.lock` is the install concurrency file, and `session-roots` records identity-verified relocated session namespaces for repair. Every published module is a validated snapshot from the stable resolver plan, including local and workspace sources.
...
<a id="monorepo-dep"></a>
**Monorepo dep**:
A local dependency whose resolved path is under the project root, usually a discovered workspace. Resolution reads it as a candidate, but publication copies the exact validated, include/exclude-filtered snapshot into `.lwpt/modules/<name>/`; later source edits require another `lwpt install`. This keeps committed zero-install state identical to the stable plan instead of exposing a live mutable link.
...
**External-path dep**:
A local dependency whose resolved path **escapes** the project root (`../../X`, absolute paths to elsewhere). Like an in-project local dependency, it is published only as the exact validated snapshot from the stable plan; the term distinguishes source location, not installation strategy. Hash verification on `--frozen` checks the committed snapshot against the lockfile.
