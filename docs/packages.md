...
The table of LWPT's canonical packages, their consumers (including LWPT,
GocciaScript, and Lakon), the divergence policy for the GocciaScript-adoption
transition window, and the bootstrap chicken-and-egg story.
...
- **Lakon consumes the CLI package as a byte-verbatim dual-compile corpus.**
  Consumer synchronization after CLI changes is tracked in
  [frostney/lakon#6](https://github.com/frostney/lakon/issues/6); re-syncing and
  running Lakon's gate happens in that consumer repository, not by editing a
  second copy in this PR.
...
|---------|----------|--------|---------------------------------------------------|
| `httpclient` | `packages/httpclient/source/HTTPClient.pas`, `HTTPClient.Test.pas`, `TransportSecurity.pas`, `TransportSecurity.Test.pas`, `FileUtils.pas`, `StringBuffer.pas`, `Tests.HTTPMockServer.pas` | Co-developed with GocciaScript; HTTPClient grew the byte-safety accumulator and body-capable POST core in LWPT; TransportSecurity owns the server-accept (memory-BIO OpenSSL on Unix-not-Darwin, native SChannel on Windows), bounded-flow, strict-identity, and safe-reload contracts; FileUtils / StringBuffer remain unchanged from GocciaScript copies | `HTTPClient.pas` diverged (LWPT ahead — byte-safety fixes and POST support); `TransportSecurity.pas` diverged (LWPT ahead — server accept, flow control, and identity lifecycle); `Tests.HTTPMockServer.pas` and `HTTPClient.Test.pas` diverged (LWPT ahead — native Windows, complete request-capture, and POST redirect regression coverage); utility units remain identical |
| `cli` | `packages/cli/source/CLI.Help.pas`, `CLI.Options.pas`, `CLI.Parser.pas`, `CLI.Parser.Test.pas`, `CLI.Subcommands.pas` (LWPT-original), `CLI.Events.pas` (LWPT-original), `CLI.Prompts.pas` (LWPT-original), `StringBuffer.pas` (bundled copy) | `CLI.*` family co-developed; LWPT canonicalised the namespace (stripped `TGoccia*` prefix from public types, dropped GocciaScript-engine-specific options as dead code, widened `CLI.Parser` to support separated long values, separated valued short options, and opt-in attached short values). Events, Subcommands, and Prompts are LWPT-original; Events is the output-neutral sequenced delivery boundary. | `CLI.Options.pas` + `CLI.Parser.pas` diverged (LWPT ahead — prefix-strip + dead-code drop + parser widening); `CLI.Help.pas` minor diverge (`{$I Goccia.inc}` → `{$I Shared.inc}`); Events + Subcommands + Prompts absent in GocciaScript |
...
| `testing` | `packages/testing/source/TestingPascalLibrary.pas`, `TestingPascalLibrary.Test.pas` (canary), `Shared.inc` (bundled copy) | Co-developed assertion + suite + runner framework. Was previously an embedded blob inside the LWPT binary served via the (now-retired) `lwpt export testing` subcommand; became a regular workspace package per [ADR-0015](./adr/0015-drop-export-testing-becomes-workspace-package.md). LWPT adds Darwin test-thread names and an opt-in, consumed active-case marker for native diagnostics. | Diverged (LWPT ahead: native diagnostic hooks) |
...
| `source/Shared.inc` | `source/Shared.inc` | Co-developed; each `packages/<name>/source/` also bundles its own copy for self-containment per [ADR-0014](./adr/0014-packages-extraction.md)'s bundled-shared-utils decision. | Identical |
...
| `source/shared/CLI.Parser.pas` | `packages/cli/source/CLI.Parser.pas` | **Diverged** (LWPT ahead) | Space-separated long-option parsing widened from repeatable-only to every valued option; short options support separated values (including values beginning with `-`) plus opt-in attached values and multi-character names; unmatched multi-character single-dash tokens remain positional; `TGoccia` prefix stripped; `AStartArg` parameter added for `lwpt run <subcommand>` aliasing |
...
| `source/shared/TestingPascalLibrary.pas` | `packages/testing/source/TestingPascalLibrary.pas` | **Diverged** (LWPT ahead) | Darwin test-thread names and the opt-in active-case marker improve native diagnostics; the marker path is consumed before test bodies so nested programs cannot overwrite the outer case. |
...
4. **Update tests in the same commit** if a public-surface change. The package's `*.Test.pas` files are the package's own contract; the root pre-merge gate (`lwpt format --check` + `lwpt build` + `lwpt agents --check` + `lwpt test`) catches regressions.
