...
|---------|----------|--------|---------------------------------------------------|
| `httpclient` | `packages/httpclient/source/HTTPClient.pas`, `HTTPClient.Test.pas`, `TransportSecurity.pas`, `TransportSecurity.Test.pas`, `FileUtils.pas`, `StringBuffer.pas`, `Tests.HTTPMockServer.pas` | Co-developed with GocciaScript; HTTPClient grew the byte-safety accumulator in LWPT; TransportSecurity owns the OpenSSL server-accept, bounded-flow, strict-identity, and safe-reload contracts; FileUtils / StringBuffer remain unchanged from GocciaScript copies | `HTTPClient.pas` diverged (LWPT ahead — byte-safety fixes); `TransportSecurity.pas` diverged (LWPT ahead — server accept, flow control, and identity lifecycle); `Tests.HTTPMockServer.pas` and `HTTPClient.Test.pas` diverged (LWPT ahead — native Windows regression coverage); utility units remain identical |
