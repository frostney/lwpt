...
| `testing` | `packages/testing/source/TestingPascalLibrary.pas`, `TestingPascalLibrary.Test.pas` (canary), `Shared.inc` (bundled copy) | Co-developed assertion + suite + runner framework. Was previously an embedded blob inside the LWPT binary served via the (now-retired) `lwpt export testing` subcommand; became a regular workspace package per [ADR-0015](./adr/0015-drop-export-testing-becomes-workspace-package.md). LWPT adds Darwin test-thread names for diagnostics. | Diverged (LWPT ahead: Darwin diagnostic thread naming) |
...
| `source/shared/TestingPascalLibrary.pas` | `packages/testing/source/TestingPascalLibrary.pas` | **Diverged** (LWPT ahead) | Darwin test-thread names improve diagnostics without changing the package's public API or test protocol. |
