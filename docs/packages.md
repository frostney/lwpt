...
|---------|----------|--------|---------------------------------------------------|
| `httpclient` | `packages/httpclient/source/HTTPClient.pas`, `TransportSecurity.pas`, `FileUtils.pas`, `StringBuffer.pas`, `Tests.HTTPMockServer.pas`, `HTTPClient.Test.pas` | Co-developed with GocciaScript; HTTPClient grew the byte-safety accumulator and the bounded-failure handling in LWPT; TransportSecurity / FileUtils / StringBuffer unchanged from GocciaScript copies | `HTTPClient.pas` diverged (LWPT ahead: byte-safety + bounded-failure fixes); rest identical |
...
|------|---------------------|--------|-------|
| `source/shared/HTTPClient.pas` | `packages/httpclient/source/HTTPClient.pas` | **Diverged** (LWPT ahead) | Four byte-safety fixes: header-recv accumulator + chunked-body seed-buffer cast. Without these, every binary download corrupts at the first `#0` byte. Plus two bounded-failure fixes: a fixed-length body cut short now raises instead of returning a zero-padded buffer the caller cannot distinguish from a complete one, and `SetPlainTransportTimeout` arms an optional read budget on the plain-socket path (off by default; TLS untouched). |
