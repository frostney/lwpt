...
|---------|----------|--------|---------------------------------------------------|
| `httpclient` | `packages/httpclient/source/HTTPClient.pas`, `TransportSecurity.pas`, `FileUtils.pas`, `StringBuffer.pas`, `Tests.HTTPMockServer.pas`, `HTTPClient.Test.pas` | Co-developed with GocciaScript; HTTPClient grew the byte-safety accumulator in LWPT; TransportSecurity / FileUtils / StringBuffer unchanged from GocciaScript copies | `HTTPClient.pas` diverged (LWPT ahead — byte-safety fixes); `Tests.HTTPMockServer.pas` and `HTTPClient.Test.pas` diverged (LWPT ahead — native Windows regression coverage); transport and utility units remain identical |
