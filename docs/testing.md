...
| **`tests/integration/InstallGitGraph.Test.pas`** | 6 tests in 1 suite | Uses deterministic advertised-ref and archive fixtures without network. Covers three-range pairwise-overlap/global-empty rejection; a queue-order regression where an initially unsatisfied shared node waits for a later transitive requirement before emitting its complete diagnostic; immutable tag/SHA identity and frozen ref-mutation immunity; a transitive constraint that changes the selected parent and forces round two while asserting one ref advertisement and one unchanged-candidate archive request; a second project materializing its missing committed archive from the first project's verified per-user object after the origin archive is removed; and a moved tag whose early lock lacks the additive commit identity, forcing one fetch of the newly advertised commit instead of unsafe reuse of the old archive object. |
...
| ---: | --- |
| **72** | **1005 Darwin / 1007 Linux / 984 Windows** |
