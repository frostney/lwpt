...
| **`source/LWPT.ObjectStore.Test.pas`** | 12 tests in 1 suite | Exercises canonical sharded SHA-256 addressing, digest rejection, verified admission/lookup/materialization, mismatch refusal, corruption quarantine, deterministic restoration when a stale lookup quarantines a concurrent valid replacement, interrupted-write invisibility, repeated fresh-store contention between two same-key producer processes, child status/stdout/stderr and durable phase diagnostics, repair overlap with an active staged admission, override normalization, and native Windows/macOS/XDG cache-root defaults. |
...
| ---: | --- |
| **69** | **940 Darwin / 942 Linux / 918 Windows** |
