...
| **`packages/httpclient/source/TransportSecurity.Test.pas`** | 39 tests in 1 suite | Pins the PKCS#12 server-accept seam against whichever backend the platform ships — memory-BIO OpenSSL on Unix-not-Darwin, native SChannel on Windows, and public Secure Transport on Darwin — with one feed/drain contract: authenticated `Active` state; independently bounded encrypted input and output; accepted-prefix admission and flow counters; stable retained output; immutable identity reload snapshots; component-safe PKCS#12 loading; strict or explicit permissive identity validation; handshake, plaintext I/O, peer and fatal closure, and graceful `close_notify`. **Gating is by backend, not by platform.** Unix-not-Darwin runs 27 cases with a raw in-memory OpenSSL client and skips 12; Windows runs 20 with a raw in-memory SChannel client and skips 19. Darwin runs 8 native cases and skips 31: its official Secure Transport client completes a TLS 1.2 handshake, bidirectional exact plaintext I/O, and graceful close entirely in memory, while lifecycle coverage recovers a definitely-dead owner's exact temporary keychain, retains an old connection snapshot across reload, aborts it, releases both isolated live keychains, and proves no file residue. Security.framework's provider-owned malformed-record timing and the existing OpenSSL-generated PKCS#12 algorithm fixtures are explicit Darwin skips rather than weakened assertions. Every functional case is offline, and the Windows and Darwin legs link no OpenSSL. |
...
| ---: | --- |
| **72** | **1019 Darwin / 1021 Linux / 998 Windows** |
