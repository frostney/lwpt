...
| **`packages/httpclient/source/TransportSecurity.Test.pas`** | 43 tests in 1 suite | Pins the PKCS#12 server-accept seam against whichever backend the platform ships — memory-BIO OpenSSL on Unix-not-Darwin, native SChannel on Windows, and public Secure Transport on Darwin — with one feed/drain contract: authenticated `Active` state; independently bounded encrypted input and output; accepted-prefix admission and flow counters; stable retained output; immutable identity reload snapshots; component-safe PKCS#12 loading; strict or explicit permissive identity validation; handshake, plaintext I/O, peer and fatal closure, and graceful `close_notify`. **Gating is by backend, not by platform.** Unix-not-Darwin runs 27 cases with a raw in-memory OpenSSL client and skips 16; Windows runs 20 with a raw in-memory SChannel client and skips 23. Darwin runs 22 native cases and skips 21: its official Secure Transport client completes TLS 1.2 handshakes and bidirectional exact plaintext I/O entirely in memory, pins bundled intermediate delivery, bounded-prefix backpressure counters, stable retained-output pointers, caller-plaintext retention plus nil/zero resume after a write-WANT, partial ciphertext in both directions, immutable reload snapshots, and TLS 1.1 rejection. Lifecycle coverage resolves public symbols under concurrent first use, disables trust network fetching, fails strict validation when the public trust evaluation rejects the bundle, injects checked keychain cleanup failures without replacing a primary error, and treats a concurrent successful residue unlink as recovery while still proving no live file residue. Deterministic status seams prove fatal errors discard retained output immediately and graceful peer close poisons the connection before reporting `peer-closed`. OpenSSL-only algorithm fixtures and backend-specific OpenSSL/SChannel operations remain explicit Darwin skips. Every functional case is offline, and the Windows and Darwin legs link no OpenSSL. |
...
| **`tests/e2e/Registry.E2E.Test.pas`** | 7 tests in 1 suite | Offline black-box coverage of `registry init` and `registry serve`: stable identity through operational reconfiguration, pre-commit rejection of invalid persisted values, remote plain-HTTP rejection, shared `--silent` classification, foreground restart, concurrent GET readers, a fixed admission budget with slow-client expiry, graceful Unix signal shutdown, a real configured TLS request through the release platform's native server backend, one current-user 0600 live keychain file, one owned residue after `SIGKILL`, bounded dead-PID recovery without following links, zero residue after graceful stop, and survival plus admission reclamation after 32 idle TLS handshakes expire. |
...
| ---: | --- |
| **72** | **1025 Darwin / 1027 Linux / 1004 Windows** |
