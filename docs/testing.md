...
| **`source/LWPT.BuildRequest.Test.pas`** | 11 tests in 1 suite | Pins canonical versioned TOML serialization and parse round-trips for build, probe, result, and capability documents; unsupported-schema failures; ordered extra arguments; compiler-independent target tuples; explicit incompatibility reasons; and normalized result validation. |
| **`source/LWPT.CompilerDriver.External.Test.pas`** | 7 tests in 1 suite | Runs a real short-lived proxy for probe and compile, proving canonical TOML stdin/stdout, live refresh, bounded raw stderr context, result/exit agreement, primary and extra-artifact confinement, timeout cleanup when a sleeping child never reads its large stdin, bounded writer cancellation when an escaped descendant retains stdin, and retain/discard drainage plus tree termination after capture overflow. |
| **`source/LWPT.CompilerRegistry.Test.pas`** | 7 tests in 1 suite | Covers build-entry/project/embedding/built-in precedence, the FPC fallback, and per-invocation caching. Factory coverage pins wrong-identity rejection, independent version enforcement, rejected-object cleanup, and exactly-once selection ownership. |
...
| **`tests/integration/CompilerProfiles.Test.pas`** | 12 tests in 1 suite | Uses a disposable external proxy through real `lwpt build` and `lwpt test` subprocesses. Covers success, entry precedence, identity/version/target failures without fallback, live capability mutation, third-probe target mutation blocking publication, malformed result diagnostics, compile timeout cleanup, artifact confinement and order-independent primary selection, and worker-capacity retention through no-edge entry postbuild, whole-build postbuild, and publication revalidation. |
...
| --- | --- | --- |
| Unit (`source/*.Test.pas` + package self-tests) | 16 | 334 |
| Integration (`tests/integration/*.Test.pas`) | 19 | 167 |
