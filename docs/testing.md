...
How LWPT tests itself: repository-owned test groups, the mock HTTP server, the binary-fetch regression that catches the byte-truncation bug, the fixture strategy, and the current test inventory.

## Executive Summary

- **Test grouping is repository policy.** LWPT's own paths distinguish co-located unit programs, `tests/integration/`, and `tests/e2e/`; the generic `lwpt test` runner does not assign or filter those meanings.
...
- **Registry lifecycle coverage is offline and cross-platform.** RFC 8032
  vectors pin signing, store tests pin immutable publication and atomic
  recovery, and a black-box E2E test exercises CLI identity, restart,
  concurrent readers, plain-HTTP policy, and a real native TLS request.
- **Status:** the repository inventory is listed below; live `lwpt test` discovery is authoritative. The framework canary ("the testing framework itself works") lives in `packages/testing/source/TestingPascalLibrary.Test.pas`.

## Repository test groups

| Group | Hits network? | Where | Runs in CI on every PR? | Runs in pre-commit hook? |
| --- | --- | --- | --- | --- |
...
LWPT_ENABLE_NETWORK=1 ./build/lwpt test 'tests/e2e/Install*.Test.pas'
```
...
| --- | --- | --- |
| **`packages/httpclient/source/HTTPClient.Test.pas`** (the headline) | 37 tests in 4 suites (Unix); 35 tests in 4 suites (Windows) | Native system hostname resolution with original Host headers and concurrent request-local results; byte-safe request and response regression, mock-server lifecycle, and response resource bounds, substantive on Unix and native Windows. Uses `packages/httpclient/source/Tests.HTTPMockServer.pas` to capture complete requests and serve crafted fixed-length, chunked, close-delimited, idle, and slow-drip responses. Pins byte-perfect embedded-`#0` handling in an 8 KiB-plus POST; authoritative generated `Content-Length` and `Content-Type`; 301/302/303 bodyless-GET redirects; 307/308 method-and-body preservation; unchanged GET/HEAD wire bytes; one-byte segmented response writes; bounded teardown before accept and during a silent accepted connection; deterministic socket/thread/WinSock accounting across repeated success, failure, and unstarted cycles; Unix `select` retry after interruption and client-fd closure on connect-wait failure; exact-limit and over-limit body/header behavior; strict and duplicate response `Content-Length` handling; malformed chunk sizes; architecture-independent rejection of a `7fffffff` chunk whose payload plus CRLF cannot fit the accumulator; fixed-length truncation; a caller-controlled redirect budget; and the monotonic whole-request deadline through both response reads and an idle TLS handshake. Includes a 32 KB response that forces multi-recv and exercises the path where header accumulation has already buffered body bytes. |
| **`packages/httpclient/source/TransportSecurity.Test.pas`** | 44 tests in 1 suite | Pins the PKCS#12 server-accept seam against whichever backend the platform ships — memory-BIO OpenSSL on Unix-not-Darwin, native SChannel on Windows, and public Secure Transport on Darwin — with one feed/drain contract: authenticated `Active` state; independently bounded encrypted input and output; accepted-prefix admission and flow counters; stable retained output; immutable identity reload snapshots; component-safe PKCS#12 loading; strict or explicit permissive identity validation; handshake, plaintext I/O, peer and fatal closure, and graceful `close_notify`. **Gating is by backend, not by platform.** Unix-not-Darwin runs 27 cases with a raw in-memory OpenSSL client and skips 17; Windows runs 20 with a raw in-memory SChannel client and skips 24. Darwin runs 23 native cases and skips 21: its official Secure Transport client completes TLS 1.2 handshakes and bidirectional exact plaintext I/O entirely in memory, pins bundled intermediate delivery, bounded-prefix backpressure counters, stable retained-output pointers, caller-plaintext retention plus nil/zero resume after a write-WANT, partial ciphertext in both directions, immutable reload snapshots, and TLS 1.1 rejection. Lifecycle coverage resolves public symbols under concurrent first use, disables trust network fetching, fails strict validation when the public trust evaluation rejects the bundle, injects checked identity-unlink failures without replacing a primary error, treats a concurrent successful residue unlink as recovery, and refuses symlink-following fixture creation. A bounded CSPRNG marker plus candidate-only reopen proves post-import continuity despite legitimate Security.framework inode replacement and path-cached keychain references. Replacement injections run before the first post-import inspection, before marker lookup, after lookup, before quarantine, and after final `FSRef` binding; bind-ABA injections cover initial construction, import, and recovery. Every replacement is preserved; only the captured owned object is unlinked. Deterministic status seams prove fatal errors discard retained output immediately and graceful peer close poisons the connection before reporting `peer-closed`. OpenSSL-only algorithm fixtures and backend-specific OpenSSL/SChannel operations remain explicit Darwin skips. Every functional case is offline, and the Windows and Darwin legs link no OpenSSL. |
...
| **`source/LWPT.Registry.Crypto.Test.pas`** | 6 tests in 1 suite | Pins RFC 8032 public-key and signature vectors, deterministic signing and verification, rejection of noncanonical encodings, and rejection of small-order public keys and signature points. |
| **`source/LWPT.Registry.Store.Test.pas`** | 42 tests in 1 suite | Covers stable origin identity through base-URL reconfiguration, signed state and restart recovery, atomic publication and checkpoint renewal, rotation interruption boundaries and uncertain retries, active-key seed selection and private permissions, staged key, transition and snapshot visibility, captured origin ancestry through concurrent activation, structurally overlapping readers, pre-commit configuration validation, committed-marker reconciliation, portable index keys, encoded and repeated-slash base paths, platform listener validation, component-safe bounded same-handle resource streaming and committed-state reads, per-short-write monotonic resource deadlines, callback-complete Network.framework teardown that preserves active primary failures, cache policy, canonical non-disclosing error responses, and stable tamper diagnostics. |
| **`source/LWPT.Registry.Mirror.Test.pas`** | 10 tests in 1 suite | Covers two-request HTTP overlap, worker and signed-byte admission limits, completed-buffer reservations, overflow-safe size arithmetic without large allocations, authenticated hash deduplication and conflicting sizes, tampered or truncated transfers with sibling draining and verified-object reuse, real-clock expiry during transfer and at a bounded post-verification activation barrier, actual CLI interruption after a completed archive pair, and bounded fixture teardown with an incomplete request under a child watchdog. |
| **`source/LWPT.Registry.Verification.Test.pas`** | 34 tests in 1 suite | Verifies the protocol corpus and real origin publication through the shared verifier: pinned trust and canonical key documents, both rotation signatures, bounded canonical rotation pages and shared acquisition metadata budgets, request-owned mirror membership with one proof verification per request and subsequent tamper rejection, package membership and complete ancestry, immutable identity through yank/restore, stale/future/equivocating checkpoints, malformed documents, pre-parser quoting and nesting bounds, shared canonical escape bytes, bounded metadata reads, cache archive identity, and complete offline proof replay after checkpoint expiry. |
| **`source/LWPT.BuildCache.Test.pas`** | 13 tests in 1 suite | Covers verified result manifests and artifact materialization, escaped artifact-kind round trips, Unix mode restoration, deterministic misses, invalid fingerprints, canonical mixed-case reference invalidation, reason-preserving corrupt-artifact rejection, budget refusal without partial logical results, deterministic distinct-result artifact eviction, both sides of concurrent store/materialize invalidation, same-result republication after stale invalidation, and concurrent cross-process publication without partial or mismatched results. |
| **`source/LWPT.CacheLifecycle.Test.pas`** | 16 tests in 1 suite (Unix); 15 tests in 1 suite (Windows) | Covers aggregate deterministic LRU eviction across cache namespaces, live-object preservation and admission refusal, empty-reference-tree removal, fail-closed reference deletion, corrupt-object and malformed-hierarchy repair with repeatable reports, transitive missing-artifact and malformed-manifest reference repair, digest verification before eviction follows a manifest, semantic index reconstruction, invalid and abandoned versus active producer-state recovery, safe Unix shard and namespace-link removal, and default/override cache-budget parsing. |
...
| **`source/LWPT.TestArtifactSet.Test.pas`** | 7 tests in 1 suite | Covers deterministic complete multi-artifact round trips, malformed-set rejection without partial output, refusal of artifacts outside or linked through the invocation-private compilation root, rejection of pre-existing destination or physical source aliases, and stable operation-stage reasons that distinguish separate exception boundaries. |
...
| **`source/LWPT.Formatter.Test.pas`** | 35 tests in 5 suites | Running `lwpt format` twice on the same file is a no-op (the contract `--check` rests on). Plus the canonical shapes that previously broke parameter-rename propagation, the uses-clause comment contract (a clause carrying `//`, `{ }`, `(* *)`, or a directive is emitted verbatim — the shapes that previously desynchronised the clause parser and rewrote the whole file as sorted "unit names"), scope-expansion coverage, and the `.lwpt/**` default-exclusion/explicit-include precedence contract, including case-sensitive include provenance on filesystems that support case-distinct paths. |
...
| **`source/LWPT.ObjectStore.Test.pas`** | 15 tests in 1 suite (Unix); 14 tests in 1 suite (Windows) | Exercises canonical sharded SHA-256 addressing, digest rejection, verified admission/lookup/materialization with exact missing/verification/copy/staged-hash failure stages, Unix readback through the owned staged handle plus close-on-exec protection serialized against the actual process-spawn boundary, mismatch refusal, corruption quarantine, deterministic restoration when a stale lookup quarantines a concurrent valid replacement, interrupted-write invisibility, rollback and recovery after a post-publication test failure, repeated fresh-store contention between two same-key producer processes, complete high-volume child stdout/stderr plus status and durable phase diagnostics, repair overlap with an active staged admission, override normalization, and native Windows/macOS/XDG cache-root defaults. |
...
| **`packages/testing/source/TestingPascalLibrary.Test.pas`** | 2 tests in 1 suite | The framework canary, lives with the package per ADR-0015. Uses TPL at arm's length so that if TPL itself breaks, this file's failure narrows the blame instead of the suite reporting opaquely. A self-spawned canary also pins opt-in active-case publication, environment consumption before test bodies, atomic case transitions, and retention of the last complete marker. |
...
| **`tests/integration/BuildClean.Test.pas`** | 6 tests in 1 suite (Unix); 5 tests in 1 suite (Windows) | Covers non-destructive clean behavior, preservation of unrelated files, a missing build directory, explicit `--no-cache` bypass after a verified hit, and Unix symlink boundaries. |
...
| **`tests/integration/InstallGitGraph.Test.pas`** | 17 tests in 1 suite | Uses deterministic advertised-ref and archive fixtures without external network access. Covers three-range pairwise-overlap/global-empty rejection; a queue-order regression where an initially unsatisfied shared node waits for a later transitive requirement before emitting its complete diagnostic; immutable tag/SHA identity and frozen ref-mutation immunity; a transitive constraint that changes the selected parent and forces round two while asserting one ref advertisement and one unchanged-candidate archive request; a second project materializing its missing committed archive from the first project's verified per-user object; exact offline reconstruction from shared cache, committed git-host and direct-URL archives, and local/workspace paths; byte-identical lock retention; refused transport; early schema-v3 named-ref and SHA compatibility; actionable lock absence, cache miss, hash corruption, manifest drift, and mutually exclusive frozen mode; pre-publication rejection of compatible constraint drift; rollback preservation of existing project state; and a moved tag whose early lock lacks the additive commit identity, forcing one fetch of the newly advertised commit instead of unsafe reuse of the old archive object. |
...
| **`tests/integration/InstallProducerLease.Test.pas`** | 2 tests in 1 suite | Exercises concurrent installs against one missing known-hash archive, proving that one producer fetches while the waiter materializes the published object, and that a waiter takes over after a producer crash without exposing partial committed state. |
...
| **`tests/integration/Repair.Test.pas`** | 8 tests in 1 suite | Spawns `lwpt repair` in scratch projects. Covers clean no-op behavior, stale install-lock removal, `.lwpt/tmp/` cleanup without touching committed module/archive state, failed build-session reclamation, dead machine-wide worker-request reclamation with diagnostics, explicit repeatable shared-cache corruption, residue, live-preservation, and byte-accounting reports, and transitive missing-artifact reference removal. |
...
| **`tests/integration/BuildSessions.Test.pas`** | 16 tests in 1 suite | Uses the test executable as a controllable FPC proxy to cover concurrent sessions, single-producer cache misses, stale publication, parallel ready entries, prerequisite publication ordering, `--jobs=1`, failure isolation, deterministic manifest-order results on Unix and Windows, routine-versus-verbose build rendering, failure diagnostics and retained logs, heartbeat observability including shared wait cadence, state-correct skipped-build diagnostics, and the fail-closed lost-proxy dispatch guard (redacted environment dump, exit 126). Unexpected observable-build exits report both FPC `TProcess` status views plus read-only worker/session-root state so an intermittent failure can be attributed without guessing from interleaved output. |
...
| **`tests/integration/TestScheduling.Test.pas`** | 28 tests in 1 suite (Unix); 24 tests in 1 suite (Windows) | Cross-platform subprocess coverage for default overlap, deterministic `--jobs=1` ordering, `--bail=0` override, compile failures counting toward bail, and the amended bail contract: stop new work, fan cancellation to active siblings under one absolute deadline, terminate and reap active children, and print sorted diagnostics. Prepared-fixture cache proofs preserve setup and behavior decisions, fingerprints, store skips, and exact artifact-set rejection reasons while bounding unavailable-cache exception details to one line. Unix runs native SIGINT/SIGTERM forwarding regressions, rejects forged acknowledgement channels backed by regular files or wrong-direction pipes, bounds control reads across data, EOF, and silent peers, and proves a nested test descendant retaining an inherited output writer cannot trap subprocess capture waiting for EOF; Windows runs matching Ctrl-C/Ctrl-Break Job Object reaping regressions. A red Windows console-controller result reports its last monotonic state and involved process IDs, while terminal acknowledgement-write failures preserve the emitter PID, byte count, and immediate Win32 error. A deterministic spawn barrier proves managed and unmanaged process creation share the inheritance-critical window. A nested owner-and-descendant fixture proves successful hop-by-hop acknowledgement and propagated `FAILED`; separate fixtures pin the post-acknowledgement reap window, an already-empty registered tree as a successful no-op, missing terminal acknowledgement, and bounded incremental/trailing protocol framing. An output-capture fixture proves the original process failure survives a secondary delegation-cleanup failure. |
| **`tests/integration/TestFlags.Test.pas`** | 3 tests in 1 suite | Spawns `lwpt test` against scratch manifests to prove ordered root `[test].flags` reach every selected compile regardless of path, LWPT-managed output arguments are rejected, and direct pretest commands inherit neither the flags nor any undeclared arguments. |
...
| File | Suites / tests | What it asserts |
| --- | --- | --- |
| **`tests/e2e/InstallGitHub.E2E.Test.pas`** | 6 tests in 1 suite | Live GitHub fetch of `octocat/Hello-World @ 7fd1a60b…` — the most stable public git ref in existence. Install exits zero, modules tree extracts under `.lwpt/modules/`, archive caches under `.lwpt/archives/<dep>-<ref>.tar.gz`, lockfile records both `archiveHash` and `computedHash`, `--frozen` re-verifies without network, **and** `--frozen` detects an archive byte-tamper (the archive-mismatch path the local-only diamond fixture cannot reach). Runs live only when `LWPT_ENABLE_NETWORK=1`. |
| **`tests/e2e/InstallGitLab.E2E.Test.pas`** | 4 tests in 1 suite | Live GitLab fetch of `gitlab-examples/ci-debug-trace @ dd648b2e48ce6518303b0bb580b2ee32fadaf045`. Validates the GitLab archive-URL pattern in `FetchURL`. Same shape as the GitHub suite: install exit / modules dir / lockfile contents / frozen reverify. Runs live only when `LWPT_ENABLE_NETWORK=1`. |
| **`tests/e2e/InstallBitbucket.E2E.Test.pas`** | 4 tests in 1 suite | Live Bitbucket fetch of `atlassian/atlaskit @ d7ac1acad54e…`. Validates the Bitbucket archive-URL pattern. Bitbucket strips the top-level dir hash-suffixed; `StripFirstComponent` handles it. Runs live only when `LWPT_ENABLE_NETWORK=1`. |
| **`tests/e2e/InstallDirectArchivesWindows.E2E.Test.pas`** | 3 tests in 1 suite | Windows-only live fetch of direct GitHub codeload + GitLab archive URLs through `lwpt install`. Bypasses source-kind URL construction so the suite specifically exercises the SChannel archive-body read path that previously corrupted `SECBUFFER_EXTRA` leftovers. Runs live only when `LWPT_ENABLE_NETWORK=1` and self-skips on non-Windows hosts. |
| **`tests/e2e/Registry.E2E.Test.pas`** | 8 tests in 1 suite | Offline black-box coverage of `registry init` and `registry serve`: stable identity through operational reconfiguration, pre-commit rejection of invalid persisted values, remote plain-HTTP rejection, shared `--silent` classification, foreground restart, concurrent GET readers, a fixed admission budget with slow-client expiry, survival after a repeated reset-on-close client storm without process-wide signal mutation, graceful Unix signal shutdown, a real configured TLS request through the release platform's native server backend, one current-user 0600 live keychain file, one owned residue after `SIGKILL`, bounded dead-PID recovery without following links, zero residue after graceful stop, and survival plus admission reclamation after 32 idle TLS handshakes expire. |
| **`tests/e2e/RegistryMirror.E2E.Test.pas`** | 10 tests in 1 suite | Exercises real origin and mirror CLI processes with binary archives: bootstrap, incremental and idempotent verified-object reuse, accepted-pointer retention and candidate metadata rejection after failure, readable activated snapshot ancestry, repeated local rotations and root-pinned bootstrap, multi-page chain retrieval, tampered or missing rotation signatures and wrong-chain rejection, retry, origin outage, mirror restart, freshness and readiness failure diagnostics, immutable root pins and exact key documents, and bounded forced child shutdown. A second origin-pinned mirror bootstraps and repeats sync from a served mirror during origin outage, verifies the same artifact identity, and rejects replayed checkpoint bytes with an altered signature. |
| **`tests/e2e/InstallScript.E2E.Test.pas`** | 13 tests in 3 suites | Unix-only. Runs `scripts/install.sh` end-to-end against the current **latest** published release — no pinned version constant. The test resolves "latest" from GET `/releases/latest` (which returns the newest non-prerelease-flagged release), passes that tag explicitly to the script, and **derives** the expected `--version` from it (binary == tag, per [ADR-0026](./adr/0026-release-version-stamp-from-tag.md)). The script curls the asset, verifies the checksum, extracts, installs; the test asserts the release resolves without a hidden failure, the install exits zero, the binary lands in `INSTALL_DIR` + is executable, and reports the resolved tag. When `GITHUB_TOKEN` is available the resolver authenticates with a Bearer header. Deterministic fake-curl suites cover authenticated resolution, explicit no-release 404, narrow connectivity skips, distinct primary/secondary rate-limit skips, hard-failing permission/server responses, unclassified curl failure, empty/malformed successful responses, and fail-closed installation when the checksums file or matching asset entry is absent. This is the suite that would have caught the macOS `.zip` regression (asset-name mismatch → 404 against a release we know exists → fail). Runs live only when `LWPT_ENABLE_NETWORK=1`; the live smoke self-skips on non-Unix, on clean connect/DNS failure (transient downtime), on a documented GitHub API rate-limit response, and on an explicit `/releases/latest` 404 until the first non-prerelease release exists (prerelease-flagged `rc.x` are covered by `release.yml`'s per-release install-smoke job instead). Other HTTP/API errors, curl failures not classified as connectivity, and parse failures fail hard. |
...
- **`tests/support/Tests.LwptSubprocess.pas`** — `TProcess` wrapper for repository E2E programs. Spawns `./build/lwpt` with given argv, captures stdout + stderr separately (no merge), supports per-test CWD + env-var overrides, and keeps live network disabled unless `LWPT_ENABLE_NETWORK=1` (`SkipNetworkTests` helper). The drain loop consumes bounded available-byte snapshots while the child runs, avoiding both pipe-capacity deadlock and an EOF wait on descendant-retained writers. Also exposes `IsNetworkUnavailable(result)` — the narrow connect/DNS-failure detector the live-network suites use to **skip on transient third-party host downtime** (a `Failed to connect to` / `Failed to resolve host` from HTTPClient) while still failing hard on a content/hash/parse mismatch. See [`ci.md`](./ci.md#transient-host-downtime-skips-it-does-not-fail).
...
- **Positional test selectors** — exact `*.Test.pas` files, recursive
  ...
  every selected program runs regardless of its path.
...
| Files | Registered test cases |
| ---: | --- |
| **75** | **1097 Darwin / 1099 Linux / 1076 Windows** |
<!-- lwpt:test-inventory-counts:end -->

## TestingPascalLibrary self-test

LWPT (and every other LWPT-using project) consumes `TestingPascalLibrary` via the `testing` workspace package, then uses it to test everything else. If `TestingPascalLibrary` breaks, none of the project's `*.Test.pas` files can tell us so. Mitigation: the `packages/testing/source/TestingPascalLibrary.Test.pas` canary exercises the framework's basic assertions and opt-in active-case diagnostic protocol through throwaway runners with distinct fatal exit codes — using TPL itself at arm's length. One file; catches the catastrophe.
