...
| **`source/LWPT.BuildSession.Test.pas`** | 16 tests in 1 suite | Covers unique private paths, bounded collision-resistant keys, atomic/stale publication, implicit, declared, and postbuild-hook input hashing, filesystem-identity publication locks, symlinked workspace inputs, and owner-guarded repair. |
...
| **`tests/integration/BuildMultiTarget.Test.pas`** | 10 tests in 1 suite | Covers named/all-target selection, fail-fast validation, private target/mode artifacts, collision-resistant job paths, non-destructive clean behavior, and continuing after per-target failures. |
...
| **`tests/integration/Hooks.Test.pas`** | 8 tests in 1 suite | Spawns build/test flows against scratch manifests with lifecycle hooks. Covers prebuild/postbuild/pretest execution, private-candidate postbuild context, failed-hook publication refusal, staleness-gated skip behavior, and dep-manifest hook dropping. |
...
| **`tests/integration/Repair.Test.pas`** | 4 tests in 1 suite | Spawns `lwpt repair` in scratch projects. Covers clean no-op behavior, stale install-lock removal, `.lwpt/tmp/` cleanup without touching committed module/archive state, and failed build-session reclamation. |
...
| --- | --- | --- |
| Unit (`source/*.Test.pas` + package self-tests) | 8 | 196 |
| Integration (`tests/integration/*.Test.pas`) | 15 | 98 |
