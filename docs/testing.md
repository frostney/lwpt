...
| **`tests/integration/BuildClean.Test.pas`** | 5 tests in 1 suite (Unix); 4 tests in 1 suite (Windows) | Covers non-destructive clean behavior, preservation of unrelated files, a missing build directory, explicit `--no-cache` bypass after a verified hit, and Unix symlink boundaries. |
