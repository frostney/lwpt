...
| **`tests/integration/InstallGitGraph.Test.pas`** | 5 tests in 1 suite | Uses deterministic advertised-ref and archive fixtures without network. Covers three-range pairwise-overlap/global-empty rejection; a queue-order regression where an initially unsatisfied shared node waits for a later transitive requirement before emitting its complete diagnostic; immutable tag/SHA identity and frozen ref-mutation immunity; a transitive constraint that changes the selected parent and forces round two while asserting one ref advertisement and one unchanged-candidate archive request; and a second project materializing its missing committed archive from the first project's verified per-user object after the origin archive is removed. |
...
| Integration | 23 | 273 Darwin / 272 Linux / 267 Windows |
| E2E | 7 | 34 Darwin, Windows / 37 Linux |
| **Total** | **63** | **868 Darwin / 870 Linux / 849 Windows** |
