...
| **`packages/httpclient/source/TransportSecurity.Test.pas`** | 9 tests in 1 suite | Pins the PKCS#12 memory-BIO server seam: path-safe missing/garbage/wrong-pass failures, reusable-context handshakes with observed want-read/want-write transitions, plaintext round-trip, retained ciphertext under partial consumption, fatal-state poisoning, `close_notify`, the TLS 1.2 floor, and the shared read-bounds clamp. Windows and Unix-not-Darwin run 8 cases and skip the Darwin stub; Darwin runs the bounds clamp + actionable Network.framework stub and skips the 7 OpenSSL cases. All functional OpenSSL coverage is pure in-memory and offline. |
...
| **`tests/integration/BuildSessions.Test.pas`** | 8 tests in 1 suite | Uses the test executable as a controllable FPC proxy to cover concurrent sessions, stale publication, parallel ready targets, prerequisite publication ordering, `--jobs=1`, failure isolation, and deterministic manifest-order results on Unix and Windows. |
...
| --- | --- | --- |
| Unit (`source/*.Test.pas` + package self-tests) | 12 | 243 |
| Integration (`tests/integration/*.Test.pas`) | 17 | 114 |
