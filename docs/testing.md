...
| **`tests/e2e/Registry.E2E.Test.pas`** | 5 tests in 1 suite | Offline black-box coverage of `registry init|serve`: stable identity through operational reconfiguration, remote plain-HTTP rejection, shared `--silent` classification, foreground restart, concurrent GET readers, a fixed admission budget with slow-client expiry, graceful Unix signal shutdown, and a real configured TLS request through the release platform's native server backend. |
...
| ---: | --- |
| **69** | **939 Darwin / 941 Linux / 917 Windows** |
