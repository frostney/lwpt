...
| **`source/LWPT.Registry.Store.Test.pas`** | 30 tests in 1 suite | Covers stable origin identity, signed state and restart recovery, atomic publication and checkpoint renewal, structurally overlapping readers, pre-commit configuration validation, committed-marker reconciliation, portable index keys, encoded and repeated-slash base paths, platform listener validation, bounded same-handle resource streaming, cache policy, canonical non-disclosing error responses, and stable tamper diagnostics. |
...
| ---: | --- |
| **72** | **999 Darwin / 1001 Linux / 976 Windows** |
