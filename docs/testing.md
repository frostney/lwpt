...
| **`tests/e2e/InstallScript.E2E.Test.pas`** | 3 tests in 1 suite | Unix-only. Runs `scripts/install.sh` end-to-end against a pinned published release (`0.1.0-rc.2`): the script curls the asset from GitHub Releases, verifies the checksum, extracts, installs the binary; the test asserts the install exits zero, the binary lands in `INSTALL_DIR` + is executable, and it runs + reports the expected version. This is the suite that would have caught the macOS `.zip` regression (asset-name mismatch → 404 → fail). Honors `LWPT_SKIP_NETWORK=1`, self-skips on non-Unix + on clean connect/DNS failure (transient downtime). See [`ci.md`](./ci.md#release-version-stamping) for why the pinned version's binary reports `0.1.0` rather than the tag. |
