...
| **`source/LWPT.CacheLifecycle.Test.pas`** | 15 tests in 1 suite (Unix); 14 tests in 1 suite (Windows) | Covers aggregate deterministic LRU eviction across cache namespaces, live-object preservation and admission refusal, empty-reference-tree removal, corrupt-object and malformed-hierarchy repair with repeatable reports, transitive missing-artifact and malformed-manifest reference repair, digest verification before eviction follows a manifest, semantic index reconstruction, invalid and abandoned versus active producer-state recovery, safe Unix shard and namespace-link removal, and default/override cache-budget parsing. |
...
| ---: | --- |
| **69** | **949 Darwin / 951 Linux / 927 Windows** |
