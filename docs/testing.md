...
| **`source/LWPT.Analysis.Pascal.Test.pas`** | 11 tests in 2 suites | Pins normalized RTL-only tokenization, byte locations, directive/comment treatment, active-style comment nesting with cross-delimiter text, actionable lexical errors, and typed separation of nested routines, declarations, program/routine bodies, explicit/implicit initialization, and finalization. Explicit initialization/finalization token ranges are disjoint from unit declarations. Representative implementation and local procedural types remain declarations rather than stealing routine bodies, and forward composite declarations do not hide later routines. |
| **`source/LWPT.Analysis.Scope.Test.pas`** | 5 tests in 1 suite | Covers recursive root/workspace ownership, globally deepest ownership when discovered project roots overlap, inherited versus replaced `[analysis]` configuration, placeholder-expanded exact build sources, additive include/final exclude behavior, deterministic root-relative ordering, strict string arrays, and supported Pascal extensions. |
...
| --- | --- | --- |
| Unit (`source/*.Test.pas` + package self-tests) | 17 | 335 |
