...
| **`source/LWPT.Duplication.Test.pas`** | 13 tests in 1 suite | Covers Type-2 clones across presentation and consistent identifier/literal renames, isolated inconsistent equality-pattern rejection, maximal non-overlapping multi-occurrence groups including same-file clones, declaration/executable and nested-routine boundaries, a 400-statement repetitive-source runtime bound, exact minimum inclusion, optional strict thresholds, validation of the 25-token floor and percentage range/type, workspace policy inheritance/replacement, and final shared-scope exclusion. |
...
| --- | --- | --- |
| Unit (`source/*.Test.pas` + package self-tests) | 19 | 366 |
| Integration (`tests/integration/*.Test.pas`) | 21 | 184 |
