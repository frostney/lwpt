...
`tests/integration/InstallFetchFailure.Test.pas` owns both halves: the pure accept/refuse table (every platform) and the wire-level failure modes driven through the real `lwpt install` subprocess (Unix-only, since the mock server is).
...
| --- | --- | --- |
| **`packages/httpclient/source/HTTPClient.Test.pas`** (the headline) | 10 tests in 2 suites | **Binary-fetch regression** (5): byte-safe `AppendRawBytes`. Uses `packages/httpclient/source/Tests.HTTPMockServer.pas` to serve crafted responses with embedded `#0` bytes in body, body-prefix (header-recv path), and chunked frames; asserts the received bytes round-trip byte-perfectly via hex comparison. Includes a 32 KB response that forces multi-recv and exercises the path where header-accumulation has already buffered some body bytes. **Bounded failure modes** (5): a fixed-length body cut short raises instead of returning a zero-padded buffer, the message names received and declared lengths, a truthful `Content-Length` still round-trips, a peer that accepts and then says nothing fails inside the armed read budget, and that budget stays disabled unless a caller arms it. |
...
| **`source/LWPT.BuildSession.Test.pas`** | 21 tests in 1 suite | Covers unique private paths, bounded collision-resistant keys, atomic/stale publication, parsed-manifest binding, implicit, declared, and postbuild-hook input hashing, filesystem-identity publication locks, symlinked workspace inputs, and owner-guarded repair. |
...
| **`tests/integration/BuildMultiTarget.Test.pas`** | 13 tests in 1 suite | Covers named/all-target selection, fail-fast target/graph/`--jobs` validation, private target/mode artifacts, collision-resistant job paths, non-destructive clean behavior, and continuing after per-target failures. |
...
| **`tests/integration/InstallFetchFailure.Test.pas`** | 19 tests in 3 suites | Spawns `lwpt install` against manifests whose dependency cannot be fetched. **Local source** (3): missing directory → exit non-zero, message names both the dep AND the missing path, `.lwpt/tmp/` empty. **Override contract** (11, every platform): `ApplyArchiveFetchOrigin` returns the canonical URL byte for byte when `LWPT_TEST_ARCHIVE_ORIGIN` is unset, swaps only the origin when set, and refuses a remote host, `localhost`, an address outside `127.0.0.0/8`, `https`, a missing port, a path, and user information. **HTTP failure modes** (5, Unix-only, since they need the mock server): HTTP 500, refused connection, a stalled peer bounded by `LWPT_TEST_ARCHIVE_TIMEOUT_MS`, and a fixed-length body cut short mid-transfer. Each asserts the dep name and the operation in the message, and that the failure commits no lockfile, cfg, cached archive, or module tree. |
...
| **`tests/e2e/InstallScript.E2E.Test.pas`** | 3 tests in 1 suite | Unix-only. Runs `scripts/install.sh` end-to-end against the current **latest** published release — no pinned version constant. The test resolves "latest" the same way `install.sh` does (GET `/releases/latest`, which returns the newest non-prerelease-flagged release), passes that tag explicitly to the script, and **derives** the expected `--version` from it (binary == tag, per [ADR-0026](./adr/0026-release-version-stamp-from-tag.md)). The script curls the asset, verifies the checksum, extracts, installs; the test asserts the install exits zero, the binary lands in `INSTALL_DIR` + is executable, and reports the resolved tag. This is the suite that would have caught the macOS `.zip` regression (asset-name mismatch → 404 against a release we know exists → fail). Honors `LWPT_SKIP_NETWORK=1`; self-skips on non-Unix, on clean connect/DNS failure (transient downtime), and until the first non-prerelease release exists (prerelease-flagged `rc.x` are covered by `release.yml`'s per-release install-smoke job instead). |
...
- **`packages/httpclient/source/Tests.HTTPMockServer.pas`**: Unix-only `TThread`-backed single-shot HTTP server. Binds an ephemeral port via `fpSocket`/`fpBind`/`fpListen`/`fpAccept`, serves caller-supplied raw response bytes (no auto-Content-Length, no implicit headers, so pathological shapes are constructible), dies after one request. Two builder helpers: `BuildSimpleResponse(body)` and `BuildChunkedResponse(chunks)`. Two states beyond "serve a response" are useful and need no extra API: constructing without calling `Start` binds and listens but never accepts, so a client connects into the kernel backlog and waits forever (the stall fixture); constructing, reading `Port`, and freeing releases a port nothing is listening on (the refused-connection fixture). The Windows path is tracked in [issue #35](https://github.com/frostney/lwpt/issues/35).
...
| --- | --- | --- |
| Unit (`source/*.Test.pas` + package self-tests) | 14 | 299 |
| Integration (`tests/integration/*.Test.pas`) | 18 | 166 |
