...
- **`tests/support/Tests.PayloadHandoff.pas`** — read-ownership transfer for a payload file one process writes and another reads. Windows makes a path visible before the writer's handle is released, so `FileExists` on the payload is not permission to open it — the failure mode behind issues [#205](https://github.com/frostney/lwpt/issues/205) and [#262](https://github.com/frostney/lwpt/issues/262), and behind the scheduling suite's Windows PID reads. `PublishReadablePayload` writes the payload and then an existence-only `<path>.complete` marker; `PayloadIsReadable` polls that marker; `ReadPayloadText` refuses to open an unpublished payload; `RetractPayload` withdraws both. `EmitPayloadCompletion` renders the same handover as Pascal source text for generated fixture programs, which cannot use the unit. The marker is never opened by the reader, so it cannot itself collide with the writer — keep it existence-only, and fix ordering rather than widening a timeout.
...
| ---: | --- |
| **72** | **1029 Darwin / 1031 Linux / 1008 Windows** |
