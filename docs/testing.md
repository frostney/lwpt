...
| **`source/LWPT.Health.Test.pas`** | 9 tests in 1 suite | Pins straight-line baselines, nested structural/nesting increments, Boolean sequences including `and then`, direct recursion without inherited-dispatch false positives, `goto`, case arms, typed handlers with fallback branches, `try/finally`, conditional routine-body alternatives, synthetic executable-section file sums, strict-greater-than maxima, complete violation collection, and canonical hotspot normalization at exact threshold boundaries. |
...
| --- | --- | --- |
| Unit (`source/*.Test.pas` + package self-tests) | 19 | 353 |
| Integration (`tests/integration/*.Test.pas`) | 18 | 155 |
| E2E (`tests/e2e/*.E2E.Test.pas` + package E2E) | 7 | 32 |
| **Total** | **44** | **544** |
