...
| **`packages/httpclient/source/TransportSecurity.Test.pas`** | 42 tests in 1 suite | Pins the PKCS#12 server-accept seam against whichever backend the platform ships — memory-BIO OpenSSL on Unix-not-Darwin, native SChannel on Windows, and public Secure Transport on Darwin — with one feed/drain contract: authenticated `Active` state; independently bounded encrypted input and output; accepted-prefix admission and flow counters; stable retained output; immutable identity reload snapshots; component-safe PKCS#12 loading; strict or explicit permissive identity validation; handshake, plaintext I/O, peer and fatal closure, and graceful `close_notify`. **Gating is by backend, not by platform.** Unix-not-Darwin runs 27 cases with a raw in-memory OpenSSL client and skips 15; Windows runs 20 with a raw in-memory SChannel client and skips 22. Darwin runs 13 native cases and skips 29: its official Secure Transport client completes a TLS 1.2 handshake and bidirectional exact plaintext I/O entirely in memory; lifecycle coverage resolves public symbols under concurrent first use, disables trust network fetching, injects checked keychain cleanup failures without replacing a primary error, recovers a definitely-dead owner's exact temporary keychain, and proves no live file residue. Deterministic status seams prove fatal errors discard retained output immediately and graceful peer close poisons the connection before reporting `peer-closed`. The existing OpenSSL-generated PKCS#12 algorithm fixtures remain explicit Darwin skips rather than weakened assertions. Every functional case is offline, and the Windows and Darwin legs link no OpenSSL. |
