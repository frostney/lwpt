...
| **`source/LWPT.CompilerRegistry.Test.pas`** | 8 tests in 1 suite | Covers build-entry/project/embedding/built-in precedence, the FPC fallback, and case-insensitive per-invocation caching. Factory coverage pins wrong-identity rejection, independent version enforcement, rejected-object cleanup, and exactly-once selection ownership. |
...
| **`tests/integration/CompilerProfiles.Test.pas`** | 13 tests in 1 suite | Uses a disposable external proxy through real `lwpt build` and `lwpt test` subprocesses. Covers success, entry precedence, identity/version/target failures without fallback, live capability mutation, third-probe target mutation blocking publication, malformed result diagnostics and per-test failure isolation, compile timeout cleanup, artifact confinement and order-independent primary selection, and worker-capacity retention through no-edge entry postbuild, whole-build postbuild, and publication revalidation. |
...
| --- | --- | --- |
| Unit (`source/*.Test.pas` + package self-tests) | 21 | 404 |
| Integration (`tests/integration/*.Test.pas`) | 22 | 197 |
