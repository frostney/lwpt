...
| **`tests/e2e/Registry.E2E.Test.pas`** | 7 tests in 1 suite | Offline black-box coverage of `registry init` and `registry serve`: stable identity through operational reconfiguration, pre-commit rejection of invalid persisted values, remote plain-HTTP rejection, shared `--silent` classification, foreground restart, concurrent GET readers, a fixed admission budget with slow-client expiry, graceful Unix signal shutdown, a real configured TLS request through the release platform's native server backend, bounded recovery of dead-process temporary-keychain storage without following links, no storage residue after readiness or `SIGKILL`, and survival plus admission reclamation after 32 idle TLS handshakes expire. |
