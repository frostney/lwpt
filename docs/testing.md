...
| **`tests/e2e/Registry.E2E.Test.pas`** | 3 tests in 1 suite | Offline black-box coverage of `registry init|serve`: stable identity through operational reconfiguration, remote plain-HTTP rejection, foreground restart, concurrent GET readers, and a real configured TLS request through the release platform's native server backend. |
