...
| **`source/LWPT.CompilerRegistry.Test.pas`** | 14 tests in 1 suite | Covers build-entry/project/embedding/built-in precedence, the FPC fallback, built-in Blaise and Lakon selection, explicit executable resolution, Lakon host-factory precedence and implicit-default replacement, script rejection, and case-insensitive per-invocation caching. Factory coverage pins wrong-identity rejection, independent version enforcement, rejected-object cleanup, exactly-once selection ownership, and refusal to shadow reserved built-in drivers. |
