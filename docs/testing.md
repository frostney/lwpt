...
| **`tests/integration/BuildClean.Test.pas`** | 4 tests in 1 suite | Covers whole-tree FPC artefact cleanup, preservation of unrelated files, a missing build directory, and Unix symlink boundaries. |
| **`tests/integration/BuildMultiTarget.Test.pas`** | 10 tests in 1 suite | Covers named/all-target selection, fail-fast validation, per-target/mode artefact isolation, cleanup, invalid target names, orphan pruning, and continuing after per-target failures. |
...
| **`tests/integration/Hooks.Test.pas`** | 6 tests in 1 suite | Spawns build/test flows against scratch manifests with lifecycle hooks. Covers prebuild/postbuild/pretest execution, staleness-gated skip behavior, and dep-manifest hook dropping. |
...
| **`tests/integration/Repair.Test.pas`** | 4 tests in 1 suite | Spawns `lwpt repair` in scratch projects. Covers clean no-op behavior, stale install-lock removal, `.lwpt/tmp/` cleanup without touching committed module/archive state, and dead machine-wide worker-request reclamation with diagnostics. |
...
| --- | --- | --- |
| Unit (`source/*.Test.pas` + package self-tests) | 9 | 206 |
| Integration (`tests/integration/*.Test.pas`) | 14 | 92 |
