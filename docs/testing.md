...
| **`source/LWPT.Registry.Store.Test.pas`** | 29 tests in 1 suite | Covers stable origin identity, signed state and restart recovery, atomic publication and checkpoint renewal, structurally overlapping readers, pre-commit configuration validation, committed-marker reconciliation, portable index keys, encoded and repeated-slash base paths, platform listener validation, streamed resource descriptors, cache policy, canonical non-disclosing error responses, and stable tamper diagnostics. |
...
| ---: | --- |
| **72** | **998 Darwin / 1000 Linux / 975 Windows** |
