...
- **Hooks fire `install` first, `build` second, `test` third when the subcommands run.** Per-subcommand ordering:
  ...
  - `lwpt build`: `[prebuild]` → for each target: per-target `prebuild` → fpc → per-target `postbuild` → publish all selected outputs → `[postbuild]`
