...
- **Compiler-neutral request first.** Build and test select a root-owned named
  ...
  the built-in fallback; Lakon has an explicit opt-in adapter; external drivers
  exchange canonical TOML over short-lived child processes. Unsupported
  combinations are hard errors, with no compiler or target fallback. See
