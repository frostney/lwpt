...
Test discovery is the exception to the general `pre*` naming intuition:
`lwpt test` freezes its discovered inventory and positional file, directory,
or glob selection before `[pretest]` runs. The hook may generate fixtures,
units, or other inputs for those programs, but a test program generated by the
hook is not part of that invocation. With no selector every discovered program
