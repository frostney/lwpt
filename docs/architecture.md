...
- **Build intent and compiler execution meet at one driver seam.**
  ...
  normalization. Root-owned profiles select built-in FPC, Delphi, or Blaise, a
...
| `[compiler]` / `[compiler.profiles.<name>]` | root-owned compiler policy. `default` names the project profile; profiles select a driver plus optional `version` and either `executable` or `script`. Built-in IDs are `fpc` and opt-in `blaise`; other IDs resolve through an embedding factory or external protocol executable. A build entry's `compiler` field overrides the project default. Dependency manifests cannot contribute this policy. |
