...
- **Error handling is production-grade.** Every multi-step install write goes through `.lwpt/tmp/` + atomic rename (EXDEV fallback to copy-then-delete), `lwpt install` takes a cross-process lock (`.lwpt/install.lock`, O_CREAT|O_EXCL), and build/test compiler outputs are private to `.lwpt/sessions/<session-id>/` until a successful build is revalidated and atomically published. See ADR-0002, ADR-0008, and ADR-0020.
...
```text
   ...
   ↑ lwpt repair operates on .lwpt/tmp/, .lwpt/install.lock, and abandoned
     .lwpt/sessions/ staging orthogonally.
