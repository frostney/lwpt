...
| Live network tests against GitLab + Bitbucket + fetch-failure-mode tests | Done | Live GitHub (`octocat/Hello-World`), GitLab (`gitlab-org/release-cli`), Bitbucket (`atlassian/atlaskit`) suites in the `tests/e2e/` tier. Fetch-failure E2E via missing-local-source; HTTP-500-via-mock-server pushed to v1.x (needs a URL-injection env-var hook). |
