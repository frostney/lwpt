...
default, E2E, and TLS slices, bounded Intel-Darwin scheduling and ordinary-path
probes, and the x86_64 Linux scheduling probe. Darwin captures native process
samples on timeout; Linux captures the active test case and `/proc` process and
thread blocking state. `full-ci`
