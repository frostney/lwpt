...
begin
  if (APath = '') or (APath[1] <> '/') then
    raise ELWPTRegistryError.CreateStable('resource_changed',
      'registry resource path must be absolute');
  CurrentDescriptor := FpOpen(PChar('/'), O_RDONLY or O_DIRECTORY_LWPT
    or O_NONBLOCK_LWPT);
  if CurrentDescriptor < 0 then
    raise ELWPTRegistryError.CreateStable('resource_changed',
      'registry resource root could not be opened safely');
  try
    Position := 1;
    ...
    begin
      ...
      if (Component = '.') or (Component = '..') then
        raise ELWPTRegistryError.CreateStable('resource_changed',
          'registry resource path is not canonical');
      while (Position <= Length(APath)) and (APath[Position] = '/') do
        Inc(Position);
      ...
      if RegistryFileStatusAt(CurrentDescriptor, PChar(Component), LinkInfo,
        ...
        raise ELWPTRegistryError.CreateStable('resource_changed',
          'registry resource component could not be inspected safely');
      if (LinkInfo.st_mode and S_IFMT) = S_IFLNK then
        raise ELWPTRegistryError.CreateStable('resource_changed',
          'registry resource path contains a link');
      if IsFinal then
      begin
        if (LinkInfo.st_mode and S_IFMT) <> S_IFREG then
          raise ELWPTRegistryError.CreateStable('resource_changed',
            'registry resource is not a regular file');
      ...
      begin
        if (LinkInfo.st_mode and S_IFMT) <> S_IFDIR then
          raise ELWPTRegistryError.CreateStable('resource_changed',
            'registry resource parent is not a directory');
        OpenFlags := O_RDONLY or O_NOFOLLOW_LWPT or O_NONBLOCK_LWPT
          or O_DIRECTORY_LWPT;
      end;
      ...
      if NextDescriptor < 0 then
        raise ELWPTRegistryError.CreateStable('resource_changed',
          'registry resource component changed while opening');
  ...
  try
    ...
    begin
      ...
      if ParentHandle = THandle(Windows.INVALID_HANDLE_VALUE) then
        raise ELWPTRegistryError.CreateStable('resource_changed',
          'registry resource parent could not be opened safely');
      ...
      begin
        ...
        raise ELWPTRegistryError.CreateStable('resource_changed',
          'registry resource parent is not a regular directory');
      end;
    ...
    if Handle = THandle(Windows.INVALID_HANDLE_VALUE) then
      raise ELWPTRegistryError.CreateStable('resource_changed',
        'registry resource could not be opened safely');
    if not Windows.GetFileInformationByHandle(Handle, FileInfo)
      ...
      raise ELWPTRegistryError.CreateStable('resource_changed',
        'registry resource is not a regular non-reparse file');
