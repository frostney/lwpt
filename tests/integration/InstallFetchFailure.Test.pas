{ InstallFetchFailure.Test — spawn `lwpt install` against a
  ...
  Failure modes covered:
    ...
    - HTTP 500, refused connection, a stalled peer bounded by the
      read budget, and a fixed-length body cut short mid-transfer.
      Each is produced by a loopback mock server and driven through the
      real `lwpt install` subprocess. These need the mock server, which
      is Unix-only until frostney/lwpt#35 lands the WinSock path, so
      that suite self-skips elsewhere.
...
{ ===========================================================================
  HTTP failure modes through the real subprocess (Unix-only mock server)
  =========================================================================== }

{$IFDEF UNIX}
const
  CRLF = #13#10;
  { The stall case alone needs a short read budget, so a peer that never
...
begin
  { Create binds and listens; Start is deliberately never called, so
    ...
    accepted or answered. Without a read budget this install hangs. }
  Mock := TMockHTTPServer.Create(nil);
  try
  ...
  Expect<Boolean>(Run.ExitCode <> 0).ToBe(True);
  Expect<Boolean>(Pos('mock-dep', Combined) > 0).ToBe(True);
  Expect<Boolean>(Pos('timed out', Combined) > 0).ToBe(True);
...
begin
  ...
  {$ELSE}
  WriteLn('install: HTTP archive-fetch failure skipped on this platform '
    + '(the mock server is Unix-only; the WinSock path is tracked '
    + 'separately).');
  {$ENDIF}
